...
bool get_anno_info(ifstream& in_anno, const int anno_choice, map<string, gene_info> & map_g_anno);
...
/*
void get_annotation(map<string, gene_info>& map_g_anno, ifstream& in_annotation, int annotype)
{
  ...
  for(iter_gene_anno_map = isoform_anno_map.begin(); iter_gene_anno_map != isoform_anno_map.end(); iter_gene_anno_map++){
    map_g_anno[(*iter_gene_anno_map).first] = gene_info( isoform_anno_map[(*iter_gene_anno_map).first] );
    }
...
int get_exon_read_count(ifstream& in_annotation, ifstream& in_sam, ofstream& out_nurd, bool if_chr, int annotype){
  ...
  // should be deleted.
  //get_anno_info(map_g_anno, in_annotation, annotype);
